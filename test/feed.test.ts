...
test('parses RSS and strips markup', () => {
  const xml = `<rss><channel><item><title><![CDATA[New &amp; useful]]></title><link>https://example.com/post?utm_source=x</link><description><![CDATA[<p>A useful first sentence.</p>]]></description><pubDate>Sun, 07 Sep 2026 10:00:00 GMT</pubDate></item></channel></rss>`;
  const [entry] = parseFeed(xml);
  assert.equal(entry.title, 'New & useful');
  assert.equal(entry.excerpt, 'A useful first sentence.');
