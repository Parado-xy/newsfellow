...
async function postWebhook(env: Env, content: string): Promise<void> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (response.status === 429 && attempt < 2) {
      const retry = await response.json<{ retry_after?: number }>().catch(() => ({}));
...
export async function sendFlaRoundup(env: Env): Promise<void> {
  const report = await collectNews(env, 'fla');
  ...
  for (const chunk of await composeDiscordDigest(stories, summarizer)) await postWebhook(env, chunk);
  console.log(JSON.stringify({ event: 'fla_roundup_delivered', ...report, stories: stories.length }));
}
